...
import (
	...
	"github.com/bwhour/go-grpc/lib/go-log/internal/lumberjack"
)
...
func write(writers []levelWriter, entries []*entry) {
	for _, entry := range entries {
		...
		if entry.useJSON {
			...
			msgUnmarshal(*(*string)(unsafe.Pointer(&msg)), &jl.Msg)
...
type defaultStdLoggerConfigurer struct {
	config       *Config
	globalLogger *defaultLogger

	stdOutput io.WriteCloser
...
func newDefaultStdLoggerConfigurer(config *Config) *defaultStdLoggerConfigurer {
	dsc := &defaultStdLoggerConfigurer{
		config: config,
...
func (dsc *defaultStdLoggerConfigurer) init() {
	dsc.stdOutput = nopWriteCloser{os.Stderr}
}

func (dsc *defaultStdLoggerConfigurer) Configure(filepath string) Logger {
	output := dsc.stdOutput

	level, ok := ParseLevel(dsc.config.Level)
	if !ok {
		level = DEBUG
	}

	logger := newDefaultLogger(level, output)
	logger.SetShowFileLine(dsc.config.ShowFileLine)
	logger.SetFormatter(dsc.config.Formatter)

	return logger
}

func (dsc *defaultStdLoggerConfigurer) Close() error {
	// dsc.exit <- struct{}{}
	// select {
	// case <-dsc.closed:
	// }
...
func (dc *defaultLoggerConfigurer) Configure(filePath string) Logger {
	...
	logger.SetFormatter(dc.config.Formatter)
...
type jsonLog struct {
	...
	Msg   map[string]string `json:"msg"` // dltag_xxx||key1=value1||key2=value2||key3=value3
...
func msgUnmarshal(str string, m *map[string]string) {
	var isDltag = true
	mp := *m
	for {
		if str == "" {
