...
// SetTraceidSource 设置 trace id 中的 source byte 并返回之前设置的值。
// 这个 source byte 用来标识一个业务线，默认是 0xda 代表 dadao。
...
// Traceid 是一个符合公司规范的 trace id。
type Traceid string

// MakeTraceid 生成一个新的 trace id。
...
// String 返回 trace id 的字符串值。
func (tid Traceid) String() string {
	return string(tid)
}

// IsValid 判断 trace id 是否合法。
// 这里只做最基本的长度判断，其他的不做验证。
func (tid Traceid) IsValid() bool {
	return len(tid) == 32
}

// IP 从 trace id 中解析出 IP 地址，如果无法解析则返回 nil。
...
// Timestamp 解析 trace id 中的时间戳，如果无法解析则返回 0。
...
// Source 返回 trace id 的 source 标识。
